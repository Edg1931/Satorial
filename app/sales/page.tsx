...
function load() {
  ...
  `).all() as Array<{ id: number; sold_at: string; customer_name: string | null; total_cents: number; payment_method: string | null; line_count: number }>;
  ...
  const totals = conn.prepare(`
    SELECT
      ...
      COUNT(*) AS orders_all
    FROM sales s
  `).get() as { rev30: number; orders30: number; rev_all: number; orders_all: number };
  ...
  `).all() as Array<{ d: string; rev: number; orders: number }>;

  const bySize = conn.prepare(`
  ...
  `).all() as Array<{ category: string; size: string; units: number; revenue: number }>;

  const byCategory = conn.prepare(`
  ...
  `).all() as Array<{ category: string; units: number; revenue: number }>;

  const aov = totals.orders30 > 0 ? totals.rev30 / totals.orders30 : 0;
  return { recent, totals, byDay, bySize, byCategory, aov };
}
