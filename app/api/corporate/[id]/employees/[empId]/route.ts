...
export async function PATCH(req: NextRequest, { params }: { params: { id: string; empId: string } }) {
  ...
  db().prepare(`UPDATE corporate_employees SET ${sets.join(", ")} WHERE id = ?`).run(...values, Number(params.empId));
  return NextResponse.json({ ok: true });
}
export async function DELETE(_req: NextRequest, { params }: { params: { id: string; empId: string } }) {
  db().prepare("DELETE FROM corporate_employees WHERE id = ?").run(Number(params.empId));
  return NextResponse.json({ ok: true });
}
